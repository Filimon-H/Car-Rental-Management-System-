    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">{t('users.fullName')}</label>
            <input
              required
              maxLength={100}
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">{t('customerCreate.email')}</label>
            <input
              required
              type="email"
              value={form.email}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-2xl bg-white shadow-xl">
        <div className="border-b border-slate-100 px-6 py-4">
          <h2 className="text-lg font-semibold text-slate-900">{t('users.resetPassword')}</h2>
          <p className="text-sm text-slate-500">{user.full_name}</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">{t('users.newPassword')}</label>
            <input
              required
              type="password"
              minLength={8}
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">{t('users.confirmPassword')}</label>
            <input
              required
              type="password"
              minLength={8}
            ...
            {confirm && password !== confirm && (
              <p className="mt-1 text-xs text-red-500">{t('users.passwordsDoNotMatch')}</p>
            )}
          </div>
          {error && (
            <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">{error}</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">
                ...
                <th className="px-4 py-3">{t('customerCreate.email')}</th>
