...
create_new_agent_file() {
    ...
    # Escape replacement strings for sed (notably '&' which expands to the matched text)
    local escaped_commands
    escaped_commands=$(printf '%s' "$commands" | sed 's/&/\\\&/g')
    local escaped_language_conventions
    escaped_language_conventions=$(printf '%s' "$language_conventions" | sed 's/&/\\\&/g')
