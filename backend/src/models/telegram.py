...
class TelegramCustomerLink(Base):
    """Telegram chat linked to a customer account."""

    __tablename__ = "telegram_customer_links"

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    customer_user_id: Mapped[int] = mapped_column(
        ForeignKey("customer_users.id"), nullable=False, unique=True, index=True
    )
    telegram_user_id: Mapped[int] = mapped_column(BigInteger, nullable=False, unique=True, index=True)
    chat_id: Mapped[int] = mapped_column(BigInteger, nullable=False, unique=True, index=True)
    telegram_username: Mapped[str | None] = mapped_column(String(255), nullable=True)
...
class TelegramCustomerLinkCode(Base):
    """One-time link code generated by an authenticated customer user."""

    __tablename__ = "telegram_customer_link_codes"
