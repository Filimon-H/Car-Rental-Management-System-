...
export default function CollateralEditPage() {
  ...
  const effectiveCustomerId = customerIdNum ?? collateral?.customer_id ?? null

  // Fetch collateral documents
  ...
  // Fetch customer info (needs effectiveCustomerId)
  const { data: customer, isLoading: loadingCustomer } = useQuery({
    queryKey: ['customer', effectiveCustomerId],
    queryFn: () => customersService.getById(effectiveCustomerId as number),
    enabled: !!effectiveCustomerId,
  })

  const getDocumentByType = (docType: DocumentType): CollateralDocument | undefined => {
    return documentsData?.items.find((doc: CollateralDocument) => doc.doc_type === docType)
  }

  const [formData, setFormData] = useState<CreateCollateralData>({
    customer_id: customerIdNum ?? 0,
    first_name: '',
    last_name: '',
    phone_primary: '',
    phone_secondary: '',
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      toast({ description: getErrorMessage(error, 'Failed to update collateral person'), variant: 'destructive' })
    },
  })
  ...
  const handleSaveAndClose = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(`/collaterals/${collateralIdNum}`)
    } catch {
  ...
  if (loadingCustomer || loadingCollateral || loadingDocuments) {
    return (
            ...
            <Section icon={<Upload className="h-5 w-5" />} title={t('customerCreate.documentUploads')}>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <DocumentDropzone
                  ...
                  onUploadComplete={() => queryClient.invalidateQueries({ queryKey: ['collateral-documents', collateralIdNum] })}
                />
              </div>
            </Section>
